...
llvm::IntrusiveRefCntPtr<llvm::vfs::FileSystem> ThreadsafeFS::view(std::string &CWD) const
{
    auto FS = view(std::nullopt);
    if (auto EC = FS->setCurrentWorkingDirectory(CWD))
        SPDLOG_ERROR("VFS: failed to set CWD to {0}: {1}", CWD, EC.message());
    return FS;
}

llvm::IntrusiveRefCntPtr<llvm::vfs::FileSystem> RealThreadsafeFS::viewImpl() const
{
